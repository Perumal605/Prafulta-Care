...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Always make openBookingModal available globally, even if config is not found
    let bookingConfig = {};
    ...
    window.openBookingModal = function openBookingModal(counsellor) {
    ...
    // Mark the function as properly initialized
    window.openBookingModal.initialized = true;
...
// Also make the function available immediately for inline onclick handlers
window.openBookingModal = function openBookingModal(counsellor) {
    // If DOM is not ready yet, wait for it
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function() {
            window.openBookingModal(counsellor);
        });
        return;
    }
    
    // If the function has been properly initialized by the DOMContentLoaded handler, use that
    if (window.openBookingModal.initialized) {
        return;
    }
    
    // Fallback: basic modal opening without full functionality
    console.warn('openBookingModal called before full initialization. Some features may not work.');
    const modal = document.getElementById('bookingOverlay');
    if (modal) {
